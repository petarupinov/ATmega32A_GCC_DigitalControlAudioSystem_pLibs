...
;;** 45. Edit on date 03.09.2015 Remove Set Clock, add Get Date/Time and Clock ***************************************;;
;;********************************************************************************************************************;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;*/
...
void RTC_1307_GET()
{
	...
	byteSecond	= ((bufferSecond/16*10) + (bufferSecond%16));	// decToBcd		// Get HEX format -> convert HEX to DEC
	byteMinute	= ((bufferMinute/16*10) + (bufferMinute%16));	// decToBcd		// Get HEX format -> convert HEX to DEC
	byteHour	= ((bufferHour/16*10) + (bufferHour%16));		// decToBcd		// Get HEX format -> convert HEX to DEC

	byteDayOfWeek = ((bufferDayOfWeek/16*10) + (bufferDayOfWeek%16));	// decToBcd		// Get HEX format -> convert HEX to DEC

	byteDate	= ((bufferDate/16*10) + (bufferDate%16));		// decToBcd		// Get HEX format -> convert HEX to DEC
	byteMonth	= ((bufferMonth/16*10) + (bufferMonth%16));		// decToBcd		// Get HEX format -> convert HEX to DEC
	byteYear	= ((bufferYear/16*10) + (bufferYear%16));		// decToBcd		// Get HEX format -> convert HEX to DEC
// -----------------------------------
...
//	uart_transmit(dayOfWeeksUart[byteDayOfWeek]);	// "MON" only with three
// ---/ OLD STYLE \---
...
//	uartTransmitDataString(' ');	// 1 space between DayOfWeek and Date
	...
	lcdDataInt(byteDate);
//	uartTransmitDataInt(byteDate);
	LCD_EXECUTE_DATA_ONE('.');
//	uartTransmitDataString('.');
	if(byteMonth<13)				// 13?? meseci 1-31
	{								// dobavqne na izkustvena '0' za desetici
		LCD_EXECUTE_DATA_ONE('0');	// dobavqne na izkustvena '0' za desetici
	}								// dobavqne na izkustvena '0' za desetici
	lcdDataInt(byteMonth);
//	uartTransmitDataInt(byteMonth);
	LCD_EXECUTE_DATA_ONE('.');
//	uartTransmitDataString('.');
	if(byteYear<10)
...
//	uartTransmitDataInt(byteMinute);
	LCD_EXECUTE_DATA_ONE(':');
//	uartTransmitDataString(':');
	if(byteSecond<10)
	{								// dobavqne na izkustvena '0', za desetici, ot 0 do 9
...
//	uartTransmitDataInt(byteSecond);
// -----------------------------------
...
// test segment begin
	...
	LCD_EXECUTE_COMMAND(LCD_SELECT_1ROW);		// select row 1
	lcdDataString("Press ENTER to set  ");		// 20 symbols on row
	LCD_EXECUTE_COMMAND(LCD_SELECT_2ROW);		// select row 2
	lcdDataString("Date and Time       ");		// 20 symbols on row
	LCD_EXECUTE_COMMAND(LCD_SELECT_3ROW);		// select row 2
	lcdDataString("or press Source     ");		// 20 symbols on row
	LCD_EXECUTE_COMMAND(LCD_SELECT_3ROW);		// select row 2
	lcdDataString("to exit             ");		// 20 symbols on row

// set time variables
	// SETUP HOURS
/*		else if((irAddress == IR_REMOTE_CAR_DEVICE_RM_X157) && (irCommand == IR_REMOTE_COMMAND_RM_X157_ENTER))	// exit hours
		{	// HOURS EXIT
			success = 0;	// break;
		}
		else if((irAddress == IR_REMOTE_CAR_DEVICE_RM_X157) && (irCommand == IR_REMOTE_COMMAND_RM_X157_SOURCE))	// EXIT DATE AND TIME
		{	// EXIT DATE AND TIME
			break;	// success = 0;
		}
		else{}
	...
	{
		if((irAddress == IR_REMOTE_CAR_DEVICE_RM_X157) && (irCommand == IR_REMOTE_COMMAND_RM_X157_UP))	// increment minutes
		{	// MINUTES INCREMENT
			minutes++;
			if(minutes > 59)
			{
		...
		else if((irAddress == IR_REMOTE_CAR_DEVICE_RM_X157) && (irCommand == IR_REMOTE_COMMAND_RM_X157_DOWN))	// decrement minutes
		{	// MINUTES DECREMENT
			minutes--;
			if(minutes < 0)
			{
		...
		else if((irAddress == IR_REMOTE_CAR_DEVICE_RM_X157) && (irCommand == IR_REMOTE_COMMAND_RM_X157_ENTER))	// exit minutes
		{	// MINUTES EXIT
			success = 0;	// break;
		}
		else if((irAddress == IR_REMOTE_CAR_DEVICE_RM_X157) && (irCommand == IR_REMOTE_COMMAND_RM_X157_SOURCE))	// EXIT DATE AND TIME
	...
	{
		if((irAddress == IR_REMOTE_CAR_DEVICE_RM_X157) && (irCommand == IR_REMOTE_COMMAND_RM_X157_UP))	// increment seconds
		{	// SECONDS INCREMENT
			seconds++;
			if(seconds > 59)
			{
		...
		else if((irAddress == IR_REMOTE_CAR_DEVICE_RM_X157) && (irCommand == IR_REMOTE_COMMAND_RM_X157_DOWN))	// decrement seconds
		{	// SECONDS DECREMENT
			seconds--;
			if(seconds < 0)
			{
		...
		else if((irAddress == IR_REMOTE_CAR_DEVICE_RM_X157) && (irCommand == IR_REMOTE_COMMAND_RM_X157_ENTER))	// exit seconds
		{	// SECONDS EXIT
			success = 0;	// break;
		}
		else if((irAddress == IR_REMOTE_CAR_DEVICE_RM_X157) && (irCommand == IR_REMOTE_COMMAND_RM_X157_SOURCE))	// EXIT DATE AND TIME
	...
	{
		if((irAddress == IR_REMOTE_CAR_DEVICE_RM_X157) && (irCommand == IR_REMOTE_COMMAND_RM_X157_UP))	// increment days
		{	// DAYS INCREMENT
			days++;
			if(days > 7)
			{
		...
		else if((irAddress == IR_REMOTE_CAR_DEVICE_RM_X157) && (irCommand == IR_REMOTE_COMMAND_RM_X157_DOWN))	// decrement days
		{	// DAYS DECREMENT
			days--;
			if(days < 1)
			{
		...
		else if((irAddress == IR_REMOTE_CAR_DEVICE_RM_X157) && (irCommand == IR_REMOTE_COMMAND_RM_X157_ENTER))	// exit days
		{	// DAYS EXIT
			success = 0;	// break;
		}
		else if((irAddress == IR_REMOTE_CAR_DEVICE_RM_X157) && (irCommand == IR_REMOTE_COMMAND_RM_X157_SOURCE))	// EXIT DATE AND TIME
	...
	{
		if((irAddress == IR_REMOTE_CAR_DEVICE_RM_X157) && (irCommand == IR_REMOTE_COMMAND_RM_X157_UP))	// increment dates
		{	// DATES INCREMENT
			dates++;
		...
		else if((irAddress == IR_REMOTE_CAR_DEVICE_RM_X157) && (irCommand == IR_REMOTE_COMMAND_RM_X157_DOWN))	// decrement dates
		{	// DATES DECREMENT
			dates--;
			if(dates < 1)
			{
		...
		else if((irAddress == IR_REMOTE_CAR_DEVICE_RM_X157) && (irCommand == IR_REMOTE_COMMAND_RM_X157_ENTER))	// exit dates
		{	// DATES EXIT
			success = 0;	// break;
		}
		else if((irAddress == IR_REMOTE_CAR_DEVICE_RM_X157) && (irCommand == IR_REMOTE_COMMAND_RM_X157_SOURCE))	// EXIT DATE AND TIME
	...
	{
		if((irAddress == IR_REMOTE_CAR_DEVICE_RM_X157) && (irCommand == IR_REMOTE_COMMAND_RM_X157_UP))	// increment months
		{	// MONTHS INCREMENT
			months++;
			if(months > 12)
			{
		...
		else if((irAddress == IR_REMOTE_CAR_DEVICE_RM_X157) && (irCommand == IR_REMOTE_COMMAND_RM_X157_DOWN))	// decrement months
		{	// MONTHS DECREMENT
			months--;
			if(months < 1)
			{
		...
		else if((irAddress == IR_REMOTE_CAR_DEVICE_RM_X157) && (irCommand == IR_REMOTE_COMMAND_RM_X157_ENTER))	// exit months
		{	// MONTHS EXIT
			success = 0;	// break;
		}
		else if((irAddress == IR_REMOTE_CAR_DEVICE_RM_X157) && (irCommand == IR_REMOTE_COMMAND_RM_X157_SOURCE))	// EXIT DATE AND TIME
	...
	{
		if((irAddress == IR_REMOTE_CAR_DEVICE_RM_X157) && (irCommand == IR_REMOTE_COMMAND_RM_X157_UP))	// increment years
		{	// YEARS INCREMENT
			years++;
			if(years > 2100)
			{
		...
		else if((irAddress == IR_REMOTE_CAR_DEVICE_RM_X157) && (irCommand == IR_REMOTE_COMMAND_RM_X157_DOWN))	// decrement years
		{	// YEARS DECREMENT
			years--;
			if(years < 2010)
			{
		...
		else if((irAddress == IR_REMOTE_CAR_DEVICE_RM_X157) && (irCommand == IR_REMOTE_COMMAND_RM_X157_ENTER))	// exit years
		{	// YEARS EXIT
			success = 0;	// break;
		}
		else if((irAddress == IR_REMOTE_CAR_DEVICE_RM_X157) && (irCommand == IR_REMOTE_COMMAND_RM_X157_SOURCE))	// EXIT DATE AND TIME
...
// HIGH and LOW BYTE ADDRESS
	...
	LCD_EXECUTE_DATA_ONE(eepromReceiveByte0);	// 9
	LCD_EXECUTE_DATA_ONE(eepromReceiveByte1);	// 10
	LCD_EXECUTE_DATA_ONE(eepromReceiveByte2);	// 11
	LCD_EXECUTE_DATA_ONE(eepromReceiveByte3);	// 12
	LCD_EXECUTE_DATA_ONE(eepromReceiveByte4);	// 13
	LCD_EXECUTE_DATA("       ", 7);				// "       " 7 SPACES do kraq na reda
}
...
void about()
{
	LCD_INIT();								// LCD INITIZLIZATION
	LCD_EXECUTE_COMMAND(LCD_SELECT_1ROW);		// select row 1
	LCD_EXECUTE_DATA(" INTELIGENT AUDIO   ",20);	// char "DATA", int 16 of chars of "DATA"
	LCD_EXECUTE_COMMAND(LCD_SELECT_2ROW);		// select row 2
	LCD_EXECUTE_DATA(" AMPLIFER with CPU  ",20);	// char "DATA", int 17 of chars of "DATA"
	LCD_EXECUTE_COMMAND(LCD_SELECT_3ROW);		// select row 3
	LCD_EXECUTE_DATA(" FW. Version beta45 ",20);	// char "DATA", int 18 of chars of "DATA"
	LCD_EXECUTE_COMMAND(LCD_SELECT_4ROW);		// select row 4
